...
改版本号后，**超级小爱输入法（com.xiaomi.type）会提示"需要 HyperOS 4.0 及以上"并拒绝工作**：
它的门禁把版本码键 `toIntOrNull` 解析，非数字串直接判死。修复方式二选一：

- 填保格式预设 `OS99.99.99.99.XPNCNXM`（本地组件能正常解析，服务器也认为你最新）
- 或给"不解析数字"的消费方单独喂真值

另一个教训：code 键（`ro.mi.os.version.code`）的消费方走 `getInt`/`toIntOrNull`，
如果对它回显完整串（如 `OS4.0.11.0.XPNCNXM`）同样会判死。本仓库模块的默认值已按 key 语义区分。
